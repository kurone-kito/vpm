...
2. If the target issue carries the configured roadmap label or an
   `vpm-roadmap-id` marker — the same test
   **A2**'s roadmap-node/execution-leaf classification rule uses (an
   ...
   - Apply **A3**'s dependency bullet to the target itself (both
     ...
     `vpm-blocked-by` markers) and its
...
NOT contain an `vpm-roadmap-id` marker (not itself
a roadmap) or an `vpm-blocked-by` marker, AND
...
`vpm-roadmap-id` marker.

**Legacy roots**: `--all-roadmaps` finds roots only by label or
`vpm-roadmap-id` marker. Retro-label a legacy
...
- **A0-T only**: the scoped body-content lookup needed to resolve
  `vpm-blocked-by` markers on the explicit target.
...
- **A3 only**: a body-content search (e.g.,
  ...
  `vpm-roadmap-id` marker when checking
  `vpm-blocked-by` dependency markers (see A3
...
- All dependency issues are closed or otherwise completed. Two forms:
  ...
  `<!-- vpm-blocked-by: {roadmap-id} -->` markers —
  find the issue whose body contains a matching
  `<!-- vpm-roadmap-id: {roadmap-id} -->`; treat as
...
`<!-- vpm-autopilot-suitability: N -->` footer, or the
...
Read the authored `<!-- vpm-effort: S|M|L -->` footer
...
- **Roadmap identity** (`vpm-roadmap-id`): in the
...
- **Sequential dependency** (`vpm-blocked-by`): in an
  issue body — this issue **cannot start until** the roadmap with the
  matching `roadmap-id` is closed.

**Do not use `vpm-blocked-by` to group sub-tasks under
